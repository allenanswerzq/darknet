...
void forward_network(network *netp)
{
    ...
    //NOTE: get network loss
    //basically collect every cost for every layer
...
{
    printf("==>starting train_network_sgd...\n");
    int batch = net->batch; 
...
{
    printf("==>starting train_network...\n");
    assert(d.X.rows % net->batch == 0);
    int batch = net->batch;
    int n = d.X.rows / batch;   //how many batches 
