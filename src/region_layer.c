...
layer make_region_layer(int batch, int w, int h, int n, int classes, int coords)
{
    ...
    l.biases = calloc(n*2, sizeof(float));  //NOTE
...
float delta_region_box(box truth, float *x, float *biases, int n, int index, int i, int j, int w, int h, float *delta, float scale, int stride)
{
    ...
    float tx = (truth.x*w - i);  //NOTE: center point x
...
void delta_region_class(float *output, float *delta, int index, int class, int classes, tree *hier, float scale, int stride, float *avg_cat, int tag)
{
    ...
    if(hier){
        ...
        *avg_cat += pred;
        ...
        for(n = 0; n < classes; ++n){
            //NOTE: if this cells belongs to a class we want that class to be one  
...
void exp_array(float *x, const int n) {
    int i;
    for (i=0; i<n; ++i) 
        x[i] = exp(x[i]);
}
...
void forward_region_layer(const layer l, network net)
{
    ...
    printf("==>forward region layer...\n");
    printf("==>anchor boxes:");
    for (int i=0; i<l.n*2; ++i)
        printf("%.5f ", l.biases[i]);
    printf("\n");
    printf("==>background: %d\n", l.background);
    printf("==>batch: %d\n", l.batch);
    ...
    for (b = 0; b < l.batch; ++b){
        for(n = 0; n < l.n; ++n){ 
            ...
            activate_array(l.output + index, 2*l.w*l.h, LOGISTIC); //NOTE: sigmod(tx) sigmod(ty)
            /*index = entry_index(l, b, n*l.w*l.h, 2);
            printf("==>tw: ");
            for (i=0; i<10; ++i)
                printf("%f ", (l.output+index)[i]);
            printf("\n");
            exp_array(l.output + index, 2*l.w*l.h); 
            */
            index = entry_index(l, b, n*l.w*l.h, l.coords); // sigmod(t0)
    ...
    if (l.softmax_tree){
        ...
        printf("==>softmax\n");
        //Calculate classes possbilities
        //NOTE
    ...
    *(l.cost) = 0;
    for (b = 0; b < l.batch; ++b) {
        ...
        for (j = 0; j < l.h; ++j) {
            for (i = 0; i < l.w; ++i) {
                for (n = 0; n < l.n; ++n) { //for each anchor box on each grid cell
                    int box_index = entry_index(l, b, n*l.w*l.h + j*l.w + i, 0);
                    box pred = get_region_box(l.output, l.biases, n, box_index, i, j, l.w, l.h, l.w*l.h);
                    float best_iou = 0;
                    for(t = 0; t < 30; ++t){ //one image at most has 30 objects or boxes.
                        box truth = float_to_box(net.truth + t*(l.coords + 1) + b*l.truths, 1);
                        if(!truth.x) break;
                        float iou = box_iou(pred, truth);
                        if (iou > best_iou) {
                    ...
                    //If this cell doesnt have an object, meaning that iou between this cell and grouth should be very low 
                    //And the network output should be zero
                    l.delta[obj_index] = l.noobject_scale * (0 - l.output[obj_index]);
                    if(l.background) l.delta[obj_index] = l.noobject_scale * (1 - l.output[obj_index]);
                    if (best_iou > l.thresh) {
                        //If this cell has an object, we need to calculate in a different way (see below code)  
                        l.delta[obj_index] = 0;
                    }
        ...
        for(t = 0; t < 30; ++t){
            ...
            printf("==>index %d %d\n",i, j);
            for(n = 0; n < l.n; ++n){
                //Cells that have an ground truth object
                int box_index = entry_index(l, b, n*l.w*l.h + j*l.w + i, 0);
                box pred = get_region_box(l.output, l.biases, n, box_index, i, j, l.w, l.h, l.w*l.h);
                if(l.bias_match){ //set to anchor boxes
                ...
                pred.x = 0;
                pred.y = 0;
                //Iou between the ground truth and anchor box
                //We need to figure out which anchor box is responsible for predicting
                float iou = box_iou(pred, truth_shift);
                if (iou > best_iou){
                    best_iou = iou;
                    best_n = n; //which anchor box has the best iou between truth box and itself.
                }
            }
            printf("==>%d %f (%f, %f) %f x %f\n", best_n, best_iou, truth.x, truth.y, truth.w, truth.h);
            //Use the best_nth to predict, meaning use network output by this predictor to calculate loss
            ...
            //This cell has an object
            l.delta[obj_index] = l.object_scale * (1 - l.output[obj_index]);
            if (l.rescore) {
                //We want network output(confidence score) to be iou 
...
//Get all boxes from network output and put all boxes into boxes array
void get_region_boxes(layer l, int w, int h, int netw, int neth, float thresh, float **probs, box *boxes, float **masks, int only_objectness, int *map, float tree_thresh, int relative)
{
    ...
    fprintf(stderr, "region layer: w %d h %d n %d\n", l.w, l.h, l.n); 
    for (i = 0; i < l.w*l.h; ++i){
        int row = i / l.w;
        int col = i % l.w;
        for(n = 0; n < l.n; ++n){
            ...
            float scale = l.background ? 1 : predictions[obj_index];//NOTE: Confidence socre iou(truth/pred)
            ...
            if(l.softmax_tree){
                ...
                for(j = 0; j < l.classes; ++j){
                    ...
                    float prob = scale*predictions[class_index];//NOTE: Pr(classi)*IOU(truth/pred)
                    probs[index][j] = (prob > thresh) ? prob : 0;
...
{
    printf("==>forward_region_layer_gpu...\n");
    copy_gpu(l.batch*l.inputs, net.input_gpu, 1, l.output_gpu, 1);
    int b, n;
    for (b = 0; b < l.batch; ++b){
        for(n = 0; n < l.n; ++n){
            int index = entry_index(l, b, n*l.w*l.h, 0);
            activate_array_gpu(l.output_gpu + index, 2*l.w*l.h, LOGISTIC);//NOTE: sigmod(x), sigmod(y)
            if(l.coords > 4){
                index = entry_index(l, b, n*l.w*l.h, 4);
                activate_array_gpu(l.output_gpu + index, (l.coords - 4)*l.w*l.h, LOGISTIC);
            }
            index = entry_index(l, b, n*l.w*l.h, l.coords);
            if(!l.background) activate_array_gpu(l.output_gpu + index,   l.w*l.h, LOGISTIC); //sigmod(t0);
            index = entry_index(l, b, n*l.w*l.h, l.coords + 1);
...
void backward_region_layer_gpu(const layer l, network net)
{
    ...
    for (b = 0; b < l.batch; ++b){
        for(n = 0; n < l.n; ++n){
            int index = entry_index(l, b, n*l.w*l.h, 0);
            gradient_array_gpu(l.output_gpu + index, 2*l.w*l.h, LOGISTIC, l.delta_gpu + index);//x and y
            if(l.coords > 4){
                index = entry_index(l, b, n*l.w*l.h, 4);
                gradient_array_gpu(l.output_gpu + index, (l.coords - 4)*l.w*l.h, LOGISTIC, l.delta_gpu + index);
            }
            index = entry_index(l, b, n*l.w*l.h, l.coords);//t0
